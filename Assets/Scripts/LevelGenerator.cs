...
public class LevelGenerator : MonoBehaviour
{
	...
	// percent of floor tiles to be pits
	public int percentChanceOfPits = 10; // in percent
	private int maxChanceOfPit = 15; // in percent
	...
	void Start()
	{
		...
		double chanceOfPit = Mathf.Min(percentChanceOfPits, maxChanceOfPit) * 0.01; // convert percent to decimal
		...
		for (int z = 0; z < mazeSize; z++)
		{
			for (int x = 0; x < mazeSize; x++)
			{
				...
				// make a random number of pits based on the percent of pits to make by not applying floor
				bool makePit = mapData[z, x] == false && Random.value < chanceOfPit;
				if (!makePit)
				{
					CreateChildPrefab(floorPrefab, floorParent, x, 0, z);
				}
